...
parser = argparse.ArgumentParser(description='This script visualizes embeddings. You can choose which classes to use for the LDA, how many components to calculate, what to visualize, and the method for dimensionality reduction. The default is to calculate the maximum number of components for the LDA, and to visualize the dendrogram, scatterplot and variance. The default method for dimensionality reduction is LDA, and the default distance metric for the dendrogram is cosine.',  epilog='The data should be in a tab delimited text file where first column is speaker, followed by columns named Component 1 onwards.')
